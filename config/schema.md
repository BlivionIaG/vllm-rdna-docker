...
2. **Implicit matrix.** The image matrix is generated by the validator from
   the Cartesian product of `[sources]` and `[bases]`, filtered by
   `source.compatible_bases`. To add a new image, add a base or a source —
   no `[[images]]` table is required and none is accepted.
3. **Exactly one default base.** Exactly one `[bases.<key>]` entry must set
   `default = true`. That base's images use unqualified tags; every other
   base's images use qualified tags (`-rocm<base.tag>`).
4. **Immutability.** Sources pin a full 40-character commit; bases pin a full
...
5. **No host state.** No absolute host paths, no ambient checkouts, no
...
6. **No side effects from validation.** The validator is read-only: it never
...
## `[bases.<key>]` (at least one required, exactly one with `default = true`)

| Field | Type | Required | Rule |
|-------|------|----------|------|
| `id` | string | yes | unique across bases |
| `default` | boolean | no | exactly one base must set this to `true` |
...
| `compatible_bases` | array<string> | yes | non-empty; every entry must be a defined base id, otherwise `UnapprovedCombination(base=..., source=...)` |

## Image matrix (implicit, no `[[images]]` table)

The validator builds the image matrix from the Cartesian product of
`[sources]` and `[bases]`, filtered by each source's `compatible_bases`.
For each surviving pair `(source, base)` the validator generates one image
record:

- **id** = `<source.id>-<base.id>` (e.g. `upstream026-rocm720`)
- **tag** rules:
  - default base + `upstream` variant: `v<source.version>` (e.g. `v0.26.0`)
  - default base + `extras-fork` variant: `v<source.version>-extras`
  - non-default base + `upstream` variant: `v<source.version>-rocm<base.tag>`
  - non-default base + `extras-fork` variant: `v<source.version>-extras-rocm<base.tag>`
- **aliasable** = the tag itself is also the canonical alias name; the
  `[aliases]` section just names one of those auto-generated records.

The matrix is fully derived from the inputs. Adding a new base or a new
source automatically extends the matrix; no `[[images]]` table entry is
required and none is accepted.

## `[aliases]` (optional)

Unqualified tag -> image id. `v0.26.0 = "upstream026-rocm720"` declares
that the unqualified tag `v0.26.0` resolves to that image. Rules:

- the target image id must exist in the auto-generated matrix (`UnknownImage`);
- the alias name must equal the target image's tag (the matrix already
  guarantees this; an explicit check is kept only for the `UnknownImage` path);
- the alias must not be in `reserved_tags` (`ReservedTag`).

Qualified tags (e.g. `v0.26.0-rocm7.14.0`) resolve through the matrix
...
| `EmptySection` | `section` | `bases`/`sources` present but empty (at least one entry required) |
...
| `MissingDefaultBase` | — | no base has `default = true` |
| `MultipleDefaultBases` | `bases` | more than one base has `default = true` |
| `UnknownBase` | `base` | patch references an undefined base |
| `UnknownImage` | `image` | alias targets an undefined (auto-generated) image |
...
| `UnapprovedCombination` | `base`, `source` | source's `compatible_bases` references a base not in `[bases]` |
| `ReservedTag` | `tag` | alias claims a reserved legacy tag |
| `DuplicateTag` | `tag` | two auto-generated images produce the same tag (conflict in the matrix) |
